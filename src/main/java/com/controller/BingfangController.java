    ...
    */
    ...
    public R save(@RequestBody BingfangEntity bingfang, HttpServletRequest request){
        ...
        Wrapper<BingfangEntity> queryWrapper = new EntityWrapper<BingfangEntity>()
            .eq("bingfang_name", bingfang.getBingfangName())
            .eq("bingfang_address", bingfang.getBingfangAddress())
            .eq("chuangwei_types", bingfang.getChuangweiTypes())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        BingfangEntity bingfangEntity = bingfangService.selectOne(queryWrapper);
        if(bingfangEntity==null){
            bingfang.setInsertTime(new Date());
...
//            return R.error(511,"永远不会进入");
    ...
    */
    ...
    public R save( String fileName){
        ...
        try {
            List<BingfangEntity> bingfangList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            BingfangEntity bingfangEntity = new BingfangEntity();
//                            bingfangEntity.setBingfangName(data.get(0));                    //病房名称 要改的
//                            bingfangEntity.setBingfangAddress(data.get(0));                    //病房位置 要改的
//                            bingfangEntity.setChuangweiTypes(Integer.valueOf(data.get(0)));   //床位 要改的
//                            bingfangEntity.setBingfangContent("");//照片
//                            bingfangEntity.setInsertTime(date);//时间
//                            bingfangEntity.setCreateTime(date);//时间
                            bingfangList.add(bingfangEntity);
                        ...
                        //查询是否重复
                        bingfangService.insertBatch(bingfangList);
                        return R.ok();
                    }
                }
            }
